...
後台新增「員工表單」分頁，內建「工作服尺寸」。員工以登入帳號填寫，一人一份，可在截止前修改。尺寸單位為 cm、體重為 kg；衣服重量選項為輕量／一般／厚重／無偏好。

管理員可新增表單（最多 20 欄，支援文字、數字、單選及必填設定）、截止／重新開放收件、查看所有回覆及匯出 CSV。發布後欄位固定；需要不同欄位時新增另一份表單。

資料存於 `forms/{formId}`，個人回覆存於 `forms/{formId}/responses/{uid}`。員工僅可讀写自己的回覆，管理員可讀取全體回覆。內建工作服表單無須預先建資料，首次切換收件狀態時才建立其表單文件。

### 上線順序

1. 將最新 `firestore.rules` 發布到 Firebase Console → intrvl-studio → Firestore Database → Rules。
2. 將 `members.html`、`assets/forms.js`、`assets/forms.css` 部署至官網。
3. 用員工帳號填寫、重新進入確認內容仍在，再用另一個員工帳號確認讀不到前者回覆。
4. 用管理員帳號確認可查看／匯出回覆、新增表單、截止收件；截止後員工仍可查看自己的回覆，但不能修改。

本機語法與模擬儲存測試不能取代部署後的 Firebase 權限驗證。
