import {initDraftPreview} from './news-draft-preview.js?v=20260924-board';
...
const updatePreview=initDraftPreview(()=>({title:$('news-title').value.trim(),body:$('news-body').value,date:$('news-date').value,url:$('news-url').value.trim(),image:source?$('crop-preview').toDataURL('image/webp',.82):savedImage}));
const status=text=>$('save-status').textContent=text;
function renderCrop(){if(source)drawCrop($('crop-preview'),source,Number($('crop-ratio').value),Number($('crop-zoom').value),Number($('crop-x').value),Number($('crop-y').value));updatePreview();}
for(const id of ['crop-ratio','crop-zoom','crop-x','crop-y'])$(id).addEventListener('input',renderCrop);
function clearImage(){imageGeneration++;source=null;savedImage='';imagePending=false;$('news-file').value='';$('crop-tools').hidden=true;updatePreview();}
...
function reset(){editId=null;clearImage();$('news-form').reset();$('news-date').value=new Date().toLocaleDateString('en-CA');$('publish-news').textContent='發佈到官網';status('');updatePreview();}
...
async function refresh(){
 $('list-status').textContent='正在載入…';try{const result=await getDocs(query(collection(db,'publicAnnouncements'),orderBy('createdAt','desc')));posts=result.docs.map(d=>({id:d.id,...d.data()}));$('published-list').replaceChildren();for(const p of posts){const article=document.createElement('article'),heading=document.createElement('h3'),date=document.createElement('p');heading.textContent=p.title;date.textContent=p.date;article.append(heading,date);if(p.image){const img=new Image();img.src=p.image;img.alt=p.title;article.append(img)}const edit=document.createElement('button');edit.type='button';edit.textContent='編輯';edit.onclick=()=>{if(busy)return;reset();editId=p.id;savedImage=p.image||'';$('news-title').value=p.title;$('news-body').value=p.body;$('news-date').value=p.date;$('news-category').value=p.category;$('news-url').value=p.url||'';$('publish-news').textContent='儲存並更新官網';status(savedImage?'保留原圖片；選擇新圖片即可重新裁切。':'');updatePreview();$('news-title').focus();};const remove=document.createElement('button');remove.type='button';remove.textContent='下架';remove.onclick=async()=>{if(busy||!admin||!confirm(`確定下架「${p.title}」？`))return;lock(true);try{await deleteDoc(doc(db,'publicAnnouncements',p.id));if(editId===p.id)reset();await refresh();status('已下架。')}catch(e){status(errorText(e))}finally{lock(false)}};article.append(edit,remove);$('published-list').append(article)}$('list-status').textContent=posts.length?'':'尚未發佈消息。'}catch(e){$('list-status').textContent=errorText(e)}
}
