 ...
 try {
  ...
  for(const lang of ['zh-TW','en','ko']) {
   const html=await page.evaluate(({template,lang})=>{
    ...
    doc.querySelectorAll('script[src]').forEach(s=>{if(/interval-ui|orken-film|instagram-posts/.test(s.src))s.setAttribute('src',s.getAttribute('src').split('?')[0]+'?v=20260924-board')});
